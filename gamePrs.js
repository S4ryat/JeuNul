...
class Player {
    ...
    attack() {
        ...
        // Après avoir attaqué, l'adversaire attaque à son tour
        setTimeout(() => {
            this.opponent.attack();
        }, 2000); // Vous pouvez ajuster ce délai selon vos préférences
    }

    // specialAttack
    specialAttack() {
        if (this.mana >= 20) {
            // Effectuez votre attaque spéciale ici
            // Par exemple, infliger 20 points de dégâts à l'adversaire
            // Réduire la mana consommée
            this.mana -= 20;
            this.card.querySelector('.mana').textContent = `Mana : ${this.mana}`;
        } else {
            // Mana insuffisante pour l'attaque spéciale
